...
__all__ = [ 'solve', 'facecube', 'permutation', 'solvable', 'CUBE_ID' ]
...
def solve(src: str, tgt: str = CUBE_ID, step: int = 30, best: bool = False) -> str:
    ...
    code = lib.solve(buffer,src_bytes,tgt_bytes,step,best)
...
def facecube(manuuver: str, cube: str = CUBE_ID) -> str: 
    ...
    buffer = ctypes.create_string_buffer(lib.CUBE_BS)
    lib.facecube(buffer,maneuver_bytes,cube_bytes)
    return buffer.value.decode('utf-8')


def permutation(ms_or_cube: str, fmt: int = 2) -> str: 
    mc = ms_or_cube.encode('utf-8')
    buffer = ctypes.create_string_buffer(lib.CUBE_BS)
    lib.permutation(buffer,mc,fmt)
    return buffer.value.decode('utf-8')
